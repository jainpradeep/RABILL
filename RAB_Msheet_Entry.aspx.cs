    ...
    { 
       //Save the uploaded Excel file.       
        ...
        else
        {
            ...
            string Path = Server.MapPath("~/Msheets");
           ...
           //First delete the existing measurement sheet from procedure
            try
            {
                ...
                if(hdrunningSrNo.Value.Length > 0)
                {
                   ...
                   objDB.executeProcedure("RABILLING.rab_msheet_Delete_RunningBill", paramUpdateList);
                }
                else
                {
                    objDB.executeProcedure("RABILLING.rab_msheet_Delete_NewBill", paramUpdateList);
                }               
            }
            catch (Exception err)
            { }
        ...
        if(gvMeasurementSheet.Rows.Count > 0)
        {
            ...
            if (objDB.executeNonQuery(sbDeleteQuery.ToString(), paramList) > 0)
            {
                ...
                try
                {
                    ...
                    paramUpdateList.Add("t_RUN_SL_NO", hdMSheetRSerialNum.Value);
                    // objDB.executeProcedure("RABILLING.rab_msheet_TBill_Update", paramUpdateList);
                    objDB.executeProcedure("RABILLING.rab_msheet_delete_Seq", paramUpdateList);                   
